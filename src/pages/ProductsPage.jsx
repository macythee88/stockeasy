...
export default function ProductsPage({ products, batches, suppliers, totalStock, upsertProduct, shout }) {
  ...
  const handlePhoto = async (e) => {
    ...
    try {
      ...
      shout('离线模式：照片暂存本地，联网后重新上传', false)
    }
    setUploading(false)
  }
  ...
  const handleSave = async () => {
    ...
    } catch { shout('保存失败', true) }
  }

  const openEdit = (p) => {
    setForm({ ...EMPTY, ...p, cost: String(p.cost||''), price: String(p.price||''),
              min_stock: String(p.min_stock||30), reorder_days: String(p.reorder_days||30) })
  ...
  if (showForm) return (
    <div>
      <button onClick={() => { setShowForm(false); setEditId(null); setForm(EMPTY) }}
        style={{ background:'none', border:'none', color:C.orange, fontWeight:700, fontSize:14, cursor:'pointer', paddingBottom:12 }}>← 返回</button>
      <div style={S.card}>
        ...
        <div style={{ textAlign:'center', marginBottom:16 }}>
          <div onClick={() => photoRef.current?.click()}
            style={{ width:88, height:88, borderRadius:14, background:C.cream, border:`2px dashed ${C.slateLight}60`,
                     margin:'0 auto 8px', cursor:'pointer', display:'flex', alignItems:'center',
                     justifyContent:'center', overflow:'hidden' }}>
            {form.photo_url
              ? <img src={form.photo_url} style={{ width:'100%', height:'100%', objectFit:'cover' }} alt="" />
          ...
          <div style={{ fontSize:10, color:C.slateLight, marginTop:4 }}>照片上传至云端，所有设备同步显示</div>
        </div>

        {/* Fields */}
        {[['产品名称 *','name','text'],['变体名称（颜色/尺寸等）','variant_name','text'],
          ['内部 SKU *','sku','text'],['售价 RM','price','number'],['进货成本 RM','cost','number'],
          ['Shopee SKU','shopee_sku','text'],['Lazada SKU','lazada_sku','text']].map(([l,k,t]) => (
          <div key={k} style={{ marginBottom:10 }}>
            <label style={S.lbl}>{l}</label>
            <input type={t} style={S.inp} value={form[k]||''} onChange={e => setForm(f => ({ ...f, [k]: e.target.value }))} />
          </div>
        ))}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:12 }}>
          <div><label style={S.lbl}>🔒 安全库存</label><input type="number" style={S.inp} value={form.min_stock} onChange={e => setForm(f => ({ ...f, min_stock: e.target.value }))} /></div>
          <div><label style={S.lbl}>📈 补货天数</label><input type="number" style={S.inp} value={form.reorder_days} onChange={e => setForm(f => ({ ...f, reorder_days: e.target.value }))} /></div>
        </div>

        {/* Supplier */}
        <div style={{ marginBottom:12 }}>
          <label style={S.lbl}>供应商</label>
          <select style={S.inp} value={form.supplier_id||''} onChange={e => setForm(f => ({ ...f, supplier_id: e.target.value }))}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
          ...
          <div><div style={{ fontSize:13, fontWeight:700 }}>⏰ 效期管理</div><div style={{ fontSize:11, color:C.slate }}>食品、保养品、药品</div></div>
          <div onClick={() => setForm(f => ({ ...f, has_expiry: !f.has_expiry }))}
            ...
            <div style={{ position:'absolute', top:2, left:form.has_expiry?22:2, width:20, height:20,
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom:12 }}>
        <input style={S.inp} placeholder="搜索产品名或 SKU…" value={searchQ}
        ...
        return (
          <div key={p.id} style={{ ...S.card, borderLeft:`3px solid ${C.orange}` }}>
            <div style={{ display:'flex', gap:10, alignItems:'center', marginBottom:10 }}>
              {p.photo_url
                ? <img src={p.photo_url} style={{ width:54, height:54, borderRadius:10, objectFit:'cover' }} alt="" />
                : <div style={{ width:54, height:54, borderRadius:10, background:C.cream, display:'flex', alignItems:'center', justifyContent:'center', fontSize:22 }}>📦</div>}
              <div style={{ flex:1 }}>
                <div style={{ fontWeight:700, fontSize:14 }}>{p.name}</div>
                <div style={{ fontSize:11, color:C.slate }}>{variants.length > 0 ? `${variants.length + 1} 个变体` : p.variant_name || '无变体'}</div>
              </div>
              ...
              return (
                <div key={v.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
                  ...
                  <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                    ...
                    <span style={{ fontSize:15, fontWeight:900, color: stock < v.min_stock ? C.red : C.navy }}>{stock}</span>
                    <button onClick={() => openEdit(v)}
                      style={{ background:'none', border:`1px solid ${C.slate}30`, borderRadius:5, padding:'2px 6px', fontSize:10, cursor:'pointer', color:C.slate }}>编辑</button>
                  </div>
                </div>
              )
            })}
            <button onClick={() => { setForm({ ...EMPTY, name:p.name, parent_id:p.id, photo_url:p.photo_url, supplier_id:p.supplier_id||'' }); setEditId(null); setShowForm(true) }}
              style={{ ...S.btn('#3498DB', true, true), marginTop:4 }}>+ 新增变体</button>
          </div>
        )
      })}
    </div>
