// src/pages/PurchasePage.jsx — 入货单扫码加入产品
...
export default function PurchasePage({ products, suppliers, purchaseOrders,
  ...
  const confirmPO = async () => {
    ...
    const po = {
      ...
      order_date: date, status: 'received', note,
    }
    try {
      await createPO(po, items.map(i => ({
        id: crypto.randomUUID(), product_id: i.product_id,
        qty: i.qty, cost: i.cost, expiry_date: i.expiry_date
      })))
  ...
  if (view === 'detail' && currentPO) {
    ...
    const poItems  = currentPO.po_items || []
    const total    = poItems.reduce((s,i) => s + i.qty*(i.cost||0), 0)
    return (
      <div>
        <button onClick={()=>{setView('list');setCurrentPO(null)}}
        ...
        <div style={S.card}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
            {[['供应商',sup?.name||'未指定'],['日期',currentPO.order_date],
              ['状态','已收货'],['总金额',`RM ${total.toFixed(2)}`]].map(([l,v])=>(
          ...
          {poItems.map((item,i)=>{
            ...
            return(
              <div key={i} style={{display:'flex',alignItems:'center',gap:10,
                  paddingBottom:10,marginBottom:10,borderBottom:i<poItems.length-1?`1px solid ${C.cream}`:'none'}}>
                {p?.photo_url&&<img src={p.photo_url} onError={e=>e.target.style.display='none'}
                  style={{width:36,height:36,borderRadius:7,objectFit:'cover'}} alt=""/>}
                <div style={{flex:1}}>
    ...
    <div>
      ...
      <div style={S.card}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
          <div><label style={S.lbl}>入货日期</label>
               <input type="date" style={S.inp} value={date} onChange={e=>setDate(e.target.value)}/></div>
          <div><label style={S.lbl}>备注</label>
               <input style={S.inp} placeholder="可选" value={note} onChange={e=>setNote(e.target.value)}/></div>
        </div>
      </div>
      ...
      <div style={S.card}>
        ...
        {/* BarcodeScanner for product lookup */}
        <BarcodeScanner
          value={scanQ}
          onChange={setScanQ}
          onScan={code => {
            setScanQ(code)
            // Auto-add after short delay to show the scanned code
            setTimeout(() => addItem(code), 200)
          }}
          placeholder="扫描条码 / SKU / 产品名… Enter"
        />

        <button type="button" onClick={()=>addItem(scanQ)}
          style={{...S.btn(C.orange,true,true),marginTop:8}}>
          ＋ 加入产品
        </button>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginTop:10}}>
          <div>
            ...
            <input type="number" min={1} style={S.inp} value={scanQty}
              onChange={e=>setScanQty(e.target.value)}/>
          </div>
          <div>
            <label style={S.lbl}>成本 RM/件</label>
            <input type="number" style={S.inp} placeholder="默认" value={scanCost}
              onChange={e=>setScanCost(e.target.value)}/>
          </div>
          <div>
            <label style={S.lbl}>有效日期</label>
            <input type="date" style={S.inp} value={scanExpiry}
              onChange={e=>setScanExpiry(e.target.value)}/>
          </div>
        </div>

        {/* Low stock quick pick */}
        {lowStock.length > 0 && (
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
              {lowStock.slice(0,6).map(p=>(
                <button key={p.id} type="button"
                  onClick={()=>{setScanQ(p.sku);setScanCost(String(p.cost))}}
                  style={{background:C.cream,border:`1px solid ${C.orange}40`,borderRadius:6,
                         padding:'4px 8px',fontSize:11,cursor:'pointer',color:C.navy}}>
                  {p.variant_name||p.name}
                  <span style={{color:C.red,fontWeight:700}}> ({totalStock(p.id)}件)</span>
                </button>
              ))}
            </div>
          </div>
      ...
      {items.length > 0 && (
        <div style={S.card}>
          ...
          {items.map((item,i)=>(
            <div key={i} style={{display:'flex',alignItems:'center',gap:10,
                paddingBottom:10,marginBottom:10,borderBottom:i<items.length-1?`1px solid ${C.cream}`:'none'}}>
              {item.product.photo_url&&<img src={item.product.photo_url}
                onError={e=>e.target.style.display='none'}
                style={{width:36,height:36,borderRadius:7,objectFit:'cover'}} alt=""/>}
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:600}}>
                  {item.product.name} <span style={{color:C.orange}}>{item.product.variant_name}</span>
                </div>
                <div style={{fontSize:11,color:C.slate}}>
                  RM {item.cost.toFixed(2)}/件{item.expiry_date?` · 效期 ${item.expiry_date}`:''}
                </div>
              </div>
              <div style={{display:'flex',alignItems:'center',gap:6}}>
                ...
                <button type="button" onClick={()=>setItems(prev=>prev.filter((_,j)=>j!==i))}
                  style={{background:'none',border:`1px solid ${C.red}`,borderRadius:5,
                         padding:'3px 7px',fontSize:10,cursor:'pointer',color:C.red}}>删</button>
              </div>
            </div>
          ))}
          ...
          <button type="button" onClick={confirmPO} style={S.btn(C.green)}>
            ✓ 确认入货，批量入库
          </button>
        </div>
      )}
  ...
  return (
    <div>
      ...
      {purchaseOrders.map(po=>{
        ...
        return(
          <div key={po.id} style={{...S.card,cursor:'pointer'}}
            onClick={()=>{setCurrentPO(po);setView('detail')}}>
            ...
            <div style={{fontSize:12,color:C.slate}}>
              {poItems.length} 种产品 · {totalQty} 件 · <strong>RM {total.toFixed(2)}</strong>
            </div>
